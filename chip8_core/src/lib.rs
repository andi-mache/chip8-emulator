...
impl Emu {
    ...
    fn execute(&mut self, op: u16) {
        ...
        match (digit1, digit2, digit3, digit4) {
            ...
            // which means we need to reset our screen buffer to be empty again.
            (0, 0, 0xE, 0) => {
                self.screen = [false; SCREEN_WIDTH * SCREEN_HEIGHT];
            },
            ...
            // JMP NNN "1NNN - Jump":The jump instruction is pretty easy to add,
            // simply move the PC to the given address.
            (1, _, _, _) => {
                let nnn = op & 0xFFF;
                self.pc = nnn;
            },
            // CALL NNN "2NNN - Call Subroutine"
            (2, _, _, _) => {
                let nnn = op & 0xFFF;
                self.push(self.pc);
                self.pc = nnn;
            },
            // SKIP VX == NN "3XNN - Skip next if VX == NN"
            (3, _, _, _) => {
            ...
            // SKIP VX != NN "4XNN - Skip next if VX != NN"
            (4, _, _, _) => {
                let x = digit2 as usize;
                let nn = (op & 0xFF) as u8;
                if self.v_reg[x] != nn {
